...
def process_question(line, anno, ans_file, cap_file, second_filter):
    assert str(line["question_id"]) == anno["image_id"]
    question_idx = line["question_id"]
    image_file = line["image"]
    question = line["text"]
    gt_answer = anno["answers"]
    ...
    qs2 = LLM_PROMPT_7.format(question.split("\nReference OCR token:")[0], answer)
    answer = llm_client_qa(qs2, max_new_tokens=1024)
    print("---------------------------------")
...
if __name__ == "__main__":
    ...
    data_path = "./eval/textvqa/llava_textvqa_val_v051_ocr.jsonl"
    annotation_path = "./eval/textvqa/TextVQA_0.5.1_val.json"
    image_folder = "/path/textvqa/train_images/"
    answers_file = f"./eval/textvqa/answers/{model_name}.jsonl"
    caption_file = f"./eval/textvqa/captions/{model_name}.jsonl"
    second_filter = args.second_filter

    questions = [json.loads(line) for line in open(data_path, "r")]
    annotations = json.load(open(annotation_path, "r"))["data"]
    ...
    # Main loop
    for line, anno in tqdm(zip(questions, annotations)):
        process_question(line, anno, ans_file, cap_file, second_filter)
